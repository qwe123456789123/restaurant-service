package org.restaurant.datatransfer;
    ...
    void process() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("C:/data/data.csv"))) {
            ...
            while ((line = br.readLine()) != null) {
                try {
                    List<String> item = mapper.readValue(line, new TypeReference<List<String>>() {
                    });
                    if (item.get(8).equals("폐업")) continue;

                    //double lat = Double.parseDouble(item.get(26));
                    //double lon = Double.parseDouble(item.get(27));
                    double[] location = transformTMToWGS84(Double.parseDouble(item.get(26)),
                            Double.parseDouble(item.get(27)));
